...
def errorbar_plot(model_name, nov, measurement, output_path):
    ...
    xtickslocs = ax.get_xticklabels()
    transform = [name_transformer(xl.get_text()) for xl in xtickslocs]
    ax.set_xticklabels(transform)
    ...
    plt.legend(loc=1)
    plt.axvspan(-0.5, 8.5, facecolor="black", alpha=0.1)
    plt.axvspan(17.5, 26.5, facecolor="black", alpha=0.1)
...
def name_transformer(original_string):
    r, d, p = original_string.split("_")
    output_raw = None
    for cha in [r, d, p]:
        if "0" not in cha:
            output_raw = cha
    # output = output_raw[1] + " Agent" + output_raw[0].upper()
    output = output_raw[1] + " " + output_raw[0].upper()
    return output
