...
4. community cards: size 5 with 53 potential values (-1 in pre-flop)
5. position: size 2
6. hole cards: size 2 with 53 potential values (-1 in pre-flop)
7. player's bankroll: size 10 (default that there are 8 players in game, -1 if no player)
8. player's action: size 10 (default that there are 8 players in game, -1 if no player)
...
from tensorflow.keras.layers import Dense, Masking, Dropout, Flatten
...
class AgentDQNReplayFrame(OweAgent):
    ...
    def _create_empty_dqn(self):
        ...
        dqn = Sequential()
        dqn.add(
            Masking(mask_value=-1, input_shape=(self.frame_size, self.state_space_size))
        )
        # dqn.add(Reshape((-1, 1)))
    ...
    def observation_encoder(self, observation, info):
        ...
        """
        """
        In this agent, I just use fully connected layer, so the order of input does not matter.
        1. game index: size 1: potential values (1~30)
        2. phase: size 1 with 4 potential values
        3. pot amount: size 1: potential values (0~1600)
        4. community cards: size 5 with 53 potential values (-1 in pre-flop)
        5. position: size 2
        6. hole cards: size 2 with 53 potential values (-1 in pre-flop)
        7. player's bankroll: size 10 (default that there are 10 players in game, -1 if no player)
        8. player's action: size 10 (default that there are 10 players in game, -1 if no player)
        """
        # 1. game_index
        ...
        observation_encoded = tf.convert_to_tensor(
            observation_encoded_list, dtype=tf.float32
        )
    ...
    def frame_processor(self):
        ...
        if cur_obs_len < self.frame_size:
            process_frame = [self.dummy_observation()] * (
                self.frame_size - cur_obs_len
            ) + self.observation_hist[-(cur_obs_len):]
        else:
    ...
    def train(self, env, seed):
        ...
        while True:
            ...
            # perform action
            observation_next, reward, terminated, truncated, info_next = env.step(
                int(user_action)
            )
            action_list = []
            ...
            encoded_observation_next = self.observation_encoder(
                observation_next, info_next
            )

            # recording tem results for plotting
            # record self.cash_his
            cur_cash = observation["bankroll"].tolist()[
                observation["position"].tolist()[1]
            ]
            ...
            if info_next["pre_game_last_observation"] != [] or terminated or truncated:
                ...
                for a_idx in range(len(self.game_his["action"])):
                    ...
                    self.replay_memory.push(
                        tem_state, tem_action, tem_reward, tem_done, tem_state_next
                    )
            ...
            if self.replay_memory.can_sample():
                ...
                tem_encoded_observation = np.array(tem_encoded_observation).reshape(
                ...
                tem_encoded_observation_next = np.array(
                    tem_encoded_observation_next
                ).reshape(len(sampled_memory), self.frame_size, -1)

                # print(self.target_dqn.predict(tem_encoded_observation_next))
                # print("---")
                # print(tf.reduce_max(self.target_dqn.predict(tem_encoded_observation_next), axis=1))
                tem_q_target_predicted = tf.reduce_max(
                    self.target_dqn.predict(tem_encoded_observation_next), axis=1
                )[:, tf.newaxis]

                tem_x = tem_encoded_observation
                tem_x = np.array(tem_x).reshape(
                    len(sampled_memory), self.frame_size, -1
                )
                tem_y = tem_action * (
                    tem_reward + self.gamma * tem_not_done * tem_q_target_predicted
                )

                self.dqn.fit(
                    tem_x, tem_y, batch_size=16, callbacks=[self.callback_mse_history]
                )
                self.mse_his += self.callback_mse_history.losses
    ...
    def get_cash_graph(self, model_name, episode_line=True):
        ...
        output_cash_episode_df = pd.DataFrame(
            {"episode_index": self.cash_episode_index}
        )
        output_cash_episode_df.to_csv(
            self._storing_path + model_name + "_cash_episode_idx.csv"
        )
    ...
    def get_mse_graph(self, model_name, episode_line=True):
        ...
        output_mse_episode_df.to_csv(
            self._storing_path + model_name + "_mse_episode_idx.csv"
        )
