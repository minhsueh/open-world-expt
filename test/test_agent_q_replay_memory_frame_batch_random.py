...
def main():
    ...
    depth = int(arg_list[4])
    test_idx = int(arg_list[5])
    # seed = 20
    random.seed(seed)
    seen = set()
    novelty_name = ""
    for d in range(depth):
        nc = random.choice(list(novelty_dict.keys()))
        while nc in seen:
            nc = random.choice(list(novelty_dict.keys()))
        seen.add(nc)
        novelty_name += random.choice(novelty_dict[nc])
        novelty_name += "+"
    novelty_name = novelty_name[:-1]
    print(novelty_name)

    global OUTPUT_FOLDER
    OUTPUT_FOLDER = f"0529_dqn_agent_testing_{test_idx}/"
    global OUTPUT_PATH
