...
def training(config_dict, param, MODEL_NAME, random_seed=15, lr=None, delta=None):
    ...
    owe_agent = AgentDQNReplayFrame(
        storing_folder_name="train_" + MODEL_NAME, alpha=lr, delta=delta
    )
    ...
    NN_novelty_inject_list = [1] * novel_tournament_count + [0] * (
        N_count - novel_tournament_count
    )
    ...
    for tournament_idx in range(NN_count + N_count):
        ...
        config_dict["log_file_path"] = (
            output_path + "tournament" + str(tournament_idx + 1) + ".log"
        )
        ...
        if game_hist[tournament_idx]["cash"][max_game_num][0] == max(
            game_hist[tournament_idx]["cash"][max_game_num]
        ):
...
def main():
    ...
    model_name = f"20260510_GameFoldRestrict_post_{param}_T{TRAIN_TOURNAMENT}_replay_frame_lr{lr}_delta{delta}"
    print(model_name)
    ...
    output_df = pd.DataFrame(
        {
            "param": param_list,
            "win_mean_list": win_mean_list,
            "win_std_list": win_std_list,
        }
    )
    output_df.to_csv(
    ...
    """
    for agent_random_num, agent_dump_num, agent_p_num in [(9, 0, 0), (0, 9, 0), (3, 3, 3)]:
        lr = 0.0001
        delta = 0.9
        lr = round(lr, 4)
        win_mean_list = []
        ...
        model_name = f"0419_{param}_T{TRAIN_TOURNAMENT}_replay_frame_lr{lr}_delta{delta}"
        print(model_name)
        win_mean, win_std = training(config_dict, param, model_name, lr=lr, delta=delta)
        param_list.append(param)
        win_mean_list.append(win_mean)
